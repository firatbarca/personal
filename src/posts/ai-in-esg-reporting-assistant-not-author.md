...
- **Extraction.** Pulling consumption figures out of thousands of utility bills and supplier PDFs that arrive in twenty different layouts — tedious for a person, fast for a model.
- **First-pass classification.** Mapping a messy expense line to the right emissions category, or flagging which ESRS datapoints a given document might touch.
- **Drafting and translation.** Turning a validated set of figures into a readable narrative, or rendering a methodology note into another working language for a local team.
- **Anomaly spotting.** Surfacing the site whose energy use halved overnight, so a human knows where to look first.
