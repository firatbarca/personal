...
| Cloudflare Workers and D1 | Public aggregate page-view counter | Normalized path, aggregate count, update time; transient rate-limit key derived by Cloudflare | No IP, user agent, fingerprint or visitor record is stored by the application | Active endpoint; local site integration pending deployment |
...
| Kit | Newsletter subscription and email delivery | Email address, subscription/consent evidence, delivery and engagement data | Delete when no longer needed; a suppression record may remain after unsubscribe | Active; double opt-in verification pending for two forms |
