...
Public documents were located, but a public document is not proof that the
correct account has accepted it. Store acceptance dates or signed copies
privately; do not commit account contracts containing personal information.
...
| Supabase | [Supabase DPA](https://supabase.com/downloads/docs/Supabase%2BDPA%2B260317.pdf) | Public DPA located; service is dormant | Retain only if the fallback remains deployed; otherwise close the project after an approved retirement decision |
| Web3Forms | No public DPA located during the review | The provider handles enquiry personal data; privacy/FAQ pages are not a substitute for an Article 28 agreement where one is required | Request its current DPA, subprocessor list, transfer mechanism and retention commitments; if unavailable, evaluate a provider that supplies them |

Review transfer safeguards and subprocessors together with each DPA. This register
does not determine whether a provider is legally suitable in every jurisdiction.
